...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        if (kp.pt.x < half_patch_size || kp.pt.y < half_patch_size || kp.pt.x >= image.cols - half_patch_size || kp.pt.y >= image.rows - half_patch_size)
        {
            // 边缘点为bad points，去除
            // keypoints.push_back({});
            continue;
        ...
        float m10 = 0, m01 = 0;
        // 计算m10 = sum x * I(x,y), m01 = sum y * I(x,y)
        ...
        // 弧度制转化为角度制     
        kp.angle = (atan2(m01, m10) / pi) * 180; // compute kp.angle 
        // END YOUR CODE HERE
    }
    return;
}
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        DescType d(256, false);     // DescType 256 bools
        for (int i = 0; i < 256; i++) {
            // START YOUR CODE HERE (~7 lines)
            // 旋转
            ...
            // rotate with theta
            cv::Point2f pp = cv::Point2f(cos(kp.angle/180*pi) * p.x - sin(kp.angle/180*pi) * p.y, sin(kp.angle/180*pi) * p.x + cos(kp.angle/180*pi) * p.y)
                         + kp.pt;
            cv::Point2f qq = cv::Point2f(cos(kp.angle/180*pi) * q.x - sin(kp.angle/180*pi) * q.y, sin(kp.angle/180*pi) * q.x + cos(kp.angle/180*pi) * q.y)
                         + kp.pt;
            // 比较
            if ((0<pp.x<image.rows)&&(0<pp.y<image.cols)&&(0<qq.x<image.rows)&&(0<qq.y<image.cols))
            {
        ...
        desc.push_back(d);      // desc是以DescType为元素的vector
    }
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for (int i1 = 0; i1 < desc1.size(); ++i1) 
    {
        ...
        for (int i2 = 0; i2 < desc2.size(); ++i2) 
        {
            ...
            int distance = 0;
            // 同时遍历desc1[i1]和desc2[i2]，元素不同则distance++
            auto it1 = desc1[i1].cbegin(), it2 = desc2[i2].cbegin();
            while (it1 != desc1[i1].cend() && it2 != desc2[i2].cend())
            {
                if (*it1 != *it2)
                {distance ++;}
                ++it1;
